<template>
  <div>
                  ...
                  <div class="simplebar-content" style="padding: 0px 0px 0px;">
                    ...
                    <div class="chat-message-list">
                      <ul class="list-unstyled chat-list chat-user-list">
                        <li v-for="(item, index) in categoryList" :key="index" @click="selectCategory(item.category, index)" :id="'category-' + index">
          ...
          <div class="w-100 overflow-hidden position-relative">
            <div class="position-relative">
              <div class="position-relative" id="channel-chat" style="display: block;">
                    ...
                    <div class="col-sm-8 col-4">
                      <ul class="list-inline user-chat-nav text-end mb-0">
                        <li class="list-inline-item m-0">
                          <div v-if="codeForm.category !== 'Medicines'" class="d-flex gap-sm-0 email-topbar-link">
                            ...
                            <button type="button" class="btn btn-ghost-secondary btn-icon  material-shadow-none" title="Delete code" @click="deleteCode">
                              <i class="bx bx-trash align-bottom fs-18"></i>
                            </button>
                          </div>
                          <div v-if="codeForm.category === 'Medicines'" class="d-flex gap-sm-0 email-topbar-link">
                            <button type="button" class="btn btn-ghost-secondary btn-icon  material-shadow-none" title="Add new medicine" @click="newMedicine">
                              <i class="bx bx-plus-circle align-bottom fs-18"></i>
                            </button>
                            <button type="button" class="btn btn-ghost-secondary btn-icon  material-shadow-none" title="Edit medicine" @click="editMedicine">
                              <i class="bx bx-edit align-bottom fs-18"></i>
                            </button>
                            <button type="button" class="btn btn-ghost-secondary btn-icon  material-shadow-none" title="Delete medicine" @click="deleteMedicine">
                              <i class="bx bx-trash align-bottom fs-18"></i>
                            </button>
                          </div>
                        </li>
                ...
                <div v-if="codeForm.category !== 'Medicines'" class="chat-conversation d-block px-3 px-lg-3 mt-lg-0" style="margin-top: 12px;">
                ...
                <div v-if="codeForm.category === 'Medicines'" class="chat-conversation d-block px-3 px-lg-3 mt-lg-0" style="margin-top: 12px;">
                  <ag-grid-vue style="height: calc(100vh - 15.0rem);" class="ag-theme-material" :columnDefs="columnMedicines" :rowData="medicines" :defaultColDef="defaultColDef" :rowHeight="36" :headerHeight="44" :suppressMenuHide="false" :suppressCellFocus="true" animateRows="false" rowSelection="single" @rowClicked="cellCicked2" @cell-double-clicked="editMedicine"></ag-grid-vue>
                </div>
              </div>
        ...
        <div class="modal-content">
          ...
          <div class="modal-footer" style="display: block;">
            <div class="hstack gap-2 justify-content-end">
              ...
              <button v-if="actMode === 'edit'" type="button" class="btn btn-success" :class="codeFormDis" @click="updateCode">Save Change</button>
            </div>
          </div>
        </div>
      </div>
    ...
    <!-- Medicines Modal -->
    <div class="modal fade zoomIn" id="medicine-modal" data-bs-keyboard="false" tabindex="-1" aria-modal="true" role="dialog" aria-hidden="true">
      <div class="modal-dialog modal-dialog-centered modal-md" role="document">
        <div class="modal-content">
          <div class="modal-header bg-light p-3">
            <h5 v-if="actMode === 'add'" class="modal-title">Add {{ codeForm.category }}</h5>
          ...
          <div class="modal-body">
            <div class="mb-3">
              ...
              <input type="text" class="form-control" placeholder="Enter medicine name" v-model="medicineForm.medicine_eng">
            </div>
            <div class="mb-3">
              <label class="form-label">ຊື່ຢາ <span class="text-danger">*</span></label>
              <input type="text" class="form-control" placeholder="ປ້ອນຊື່ຢາພາສາລາວ" v-model="medicineForm.medicine_lao">
            </div>
            <div class="row g-3">
              <div class="col-lg-4 mb-3">
                <label class="form-label">Unit <span class="text-danger">*</span></label>
                <multiselect :searchable="true" :searchStart="false" placeholder="Select unit" v-model="medicineForm.unit_id" :options="lkUnit" />
              </div>
              <div class="col-lg-4 mb-3">
                <label class="form-label">Unit Price </label>
                <input type="number" class="form-control" placeholder="Unit price" v-model="medicineForm.unit_price">
              </div>
              <div class="col-lg-4 mb-3">
                <label class="form-label">Remind Number </label>
                <input type="number" class="form-control" placeholder="Reminder" v-model="medicineForm.min_remind">
              </div>
            </div>
            <div class="mb-0">
              <label class="form-label">Description <span class="text-danger">*</span></label>
              <textarea class="form-control" rows="3" placeholder="Enter description" v-model="medicineForm.descr"></textarea>
            </div>
          </div>
          <div class="modal-footer" style="display: block;">
            <div class="hstack gap-2 justify-content-end">
              <button type="button" class="btn btn-light" data-bs-dismiss="modal">Cancel</button>
              <button v-if="actMode === 'add'" type="button" class="btn btn-success" :class="medicineFormDis" @click="addMedicine">Add Medicine</button>
              <button v-if="actMode === 'edit'" type="button" class="btn btn-success" :class="medicineFormDis" @click="updateMedicine">Save Change</button>
            </div>
          </div>
        </div>
      </div>
          ...
          <div class="modal-body text-center p-5">
            ...
            <div class="text-center">
              ...
              <div class="hstack gap-2 justify-content-center remove">
                ...
                <button class="btn btn-soft-danger" @click="confirmDelete">Yes, Delete It</button>
              </div>
            </div>
          </div>
        </div>
...
export default {
  ...
  data() {
    return {
      ...
      codeForm: { code_id: '', category: '', code: '', code_old: '', descr_eng: '', descr_lao: '', active: 1, tbl_name: '', col_name: '' },
      medicineForm: { medicine_id: '', medicine_eng: '', medicine_lao: '', descr: '', unit_id: null, unit_eng: '', unit_lao: '', unit_price: '', min_remind: '' },
  ...
  computed: {
    categoryList() {
      ...
      return searchTerm ? this.categories.filter(e => e.category.toLowerCase().startsWith(searchTerm)) : this.categories;
    },

    codeFormDis() {
      return (this.codeForm.code && this.codeForm.descr_eng && this.codeForm.descr_lao && this.codeForm.active !== null) ? '' : 'disabled';
    },

    medicineFormDis() {
      return (this.medicineForm.medicine_eng && this.medicineForm.medicine_lao && this.medicineForm.unit_id && this.medicineForm.descr) ? '' : 'disabled';
    },
  },
  ...
  methods: {
    ...
    async getCategories() {
      const { data } = await axios.get('api/clinic/categories', { headers: { Authorization: 'Bearer ' + this.authStore.getToken } });
      this.categories = data;
    },

    async getCodes() {
      const { data } = await axios.get('api/clinic/codes', { headers: { Authorization: 'Bearer ' + this.authStore.getToken } });
      this.codes = data;

      this.lkUnit = this.codes.filter(e => e.category === 'Unit')
        .map(({ code_id, descr_eng }) => ({ value: code_id, label: descr_eng }));
    },
    ...
    async selectCategory(cate, inx) {
      ...
      const { tbl_name, col_name } = this.categories.find(e => e.category === cate);
    ...
    newCode() {
      ...
      this.actMode = 'add';
      Object.assign(this.codeForm, { code_id: '', code: '', descr_eng: '', descr_lao: '', active: 1 });
      $('#code-modal').modal('show');
    },

    async addCode() {
      try {
        const { data } = await axios.post('api/clinic/add-code', this.codeForm, { headers: { Authorization: 'Bearer ' + this.authStore.getToken } });
        if (data.success) {
          await this.getCodes();
          this.codeFilter = this.codes.filter(e => e.category == this.codeForm.category);
          this.codeForm.code_id = '';
    ...
    async updateCode() {
      try {
        await axios.post('api/clinic/upd-code', this.codeForm, { headers: { Authorization: 'Bearer ' + this.authStore.getToken } });
        await this.getCodes();
        this.codeFilter = this.codes.filter(e => e.category == this.codeForm.category);
        this.codeForm.code_id = '';
        $('#code-modal').modal('hide');
    ...
    async confirmDelete() {
      try {
        const url = this.codeForm.category == 'Medicines' ? 'api/clinic/del-medicine' : 'api/clinic/del-code';
        const data = this.codeForm.category == 'Medicines' ? this.medicineForm : this.codeForm;
        await axios.post(url, data, { headers: { Authorization: 'Bearer ' + this.authStore.getToken } });

        this.codeForm.category == 'Medicines' ? await this.getMedicines() : await this.getCodes();
        this.codeFilter = this.codes.filter(e => e.category == this.codeForm.category);
        this.codeForm.code_id = '';
        this.medicineForm.medicine_id = '';
    ...
    newMedicine() {
      if (this.auth.new == 0) return toastr.warning("You're not authorized to add.");
      this.actMode = 'add';
      Object.assign(this.medicineForm, { medicine_eng: '', medicine_lao: '', descr: '', unit_id: null, unit_price: '', min_remind: '' });
      $('#medicine-modal').modal('show');
    },

    async addMedicine() {
      try {
        const { data } = await axios.post('api/clinic/add-medicine', this.medicineForm, { headers: { Authorization: 'Bearer ' + this.authStore.getToken } });
        if (data.success) {
          await this.getMedicines();
          this.medicineForm.medicine_id = '';
    ...
    editMedicine() {
      if (this.auth.edit == 0) return toastr.warning("You're not authorized to edit.");
      if (!this.medicineForm.medicine_id) return toastr.info("Select a row to edit.");
      this.actMode = 'edit';
      $('#medicine-modal').modal('show');
    },
    ...
    async updateMedicine() {
      try {
        await axios.post('api/clinic/upd-medicine', this.medicineForm, { headers: { Authorization: 'Bearer ' + this.authStore.getToken } });
        await this.getMedicines();
        this.medicineForm.medicine_id = '';
        $('#medicine-modal').modal('hide');
        toastr.success('Successfully!');
    ...
    deleteMedicine() {
      if (this.auth.del == 0) return toastr.warning("You're not authorized to delete.");
      if (!this.medicineForm.medicine_id) return toastr.info("Select a row to delete.");
      $('#delete-modal').modal('show');
    },
